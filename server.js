...
if (pomboCount === 0) {
  ...
  const tiposIniciais = [
    ...
    ['pombo_cosmico', 'Pombo Cósmico', 40000, 5, 1000, '🌌', 'Reservado para distâncias absurdas (>5000km). Viaja entre dimensões.'],
...
app.post('/api/users', (req, res) => {
  ...
  const info = db.prepare(`
    ...
    VALUES (?, ?, ?, ?)
  `).run(username, hash, latitude || 0, longitude || 0);
...
// Atualizar localização (simulada)
...
app.post('/api/messages', authMiddleware, (req, res) => {
  ...
  // Pombo cósmico obrigatório para distâncias absurdas
  if (distancia > 5000 && tipoPombo !== 'pombo_cosmico') {
    return res.status(400).json({
      error: 'Distância maior que 5000km! Você precisa usar o Pombo Cósmico para essa entrega.',
      distancia
    });
  }
  ...
  if (distancia < 0.5) {
    ...
    // clamp de segurança: min 2s, max 6h (pra nao travar UX em casos extremos)
    tempoChegada = Math.max(2000, Math.min(tempoChegada, 6 * 60 * 60 * 1000));
  }
